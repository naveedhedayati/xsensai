...
**Completed:** v0.9.2.0 (2026-06-02). Fine-grained-PAT writeback + canary preflight + `SYNC_TOKEN_PERSIST_FAILED` flag shipped. Follow-ups: symmetric dual-write (P1) + canary propagation test (P3), both filed below.
**Priority:** P0
**Origin:** Manual QA pass 2026-04-29 (Phase D verify-after-fix). X rotates the OAuth 2.0 refresh token on every successful refresh and invalidates the previous one. The cron uses `EnvSecretTokenProvider`, whose `store_refresh_token` is documented as a soft no-op because the runner can't write back to its own GH secret in real time. Concrete failure observed: cron run #25130371697 succeeded, X returned new RT in-memory, cron exited and the RT was lost; cron run #25130974750 (10 minutes later, also from `gh workflow run`) failed `[AUTH_FAILED]`. Net effect: **the cron can run successfully exactly once between user-initiated `setup_oauth.py` re-auths.** The 2-day cron schedule is functionally a 2-day-MTBF system requiring manual user re-auth.
**Description:** Implement secret rotation in headless: after a successful refresh, encrypt the new RT against the xsensai repo's Actions public key (`gh api repos/<owner>/<repo>/actions/secrets/public-key`) and PUT it via `repos/<owner>/<repo>/actions/secrets/XSENSAI_X_REFRESH_TOKEN`. Requires the cron's GITHUB_TOKEN to carry `secrets:write` scope (currently the workflow has `permissions: contents: read` only — needs to add `actions: write` or similar; verify exact scope name in GH docs). New module: `xsensai.sync.gh_secrets_updater`. Existing `EnvSecretTokenProvider.store_refresh_token` no-op stays as the fallback; the cron path explicitly calls the updater after each rotation. The change closes the rotation loop and makes the cron self-sustaining — the only remaining manual touch is when the X dev portal forces a token revocation.
**Files:** new `src/xsensai/sync/gh_secrets_updater.py`, [src/xsensai/entrypoints/headless.py](src/xsensai/entrypoints/headless.py) (call updater on `X rotated the refresh token` log point), `.github/workflows/sync.yml` (add `actions: write` permission), [docs/CRON_SETUP.md](docs/CRON_SETUP.md) (note self-rotation is automatic post-this-fix).
**Update 2026-06-02 (whole-project test pass — routed to design pass, not yet implemented):** The predicted failure mode is now chronic. The last 6 consecutive scheduled cron runs all fail `[AUTH_FAILED]` exit 2 (through 2026-06-01: runs 26755087142, 26709255011, 26632682732, 26506797090, 26397330391, 26328717701); heartbeat shows `last_cron_success: null`. A *secondary* issue surfaced in the same logs — the cron's vault clone was skipping ~4 cards on `CardFrontmatter` validation because the remote vault still had old v1-schema frontmatter; **RESOLVED** by migrating + pushing those cards (LifeOS_v2 commit 592b6dc, 2026-06-02). So the RT rotation above is now the *sole* remaining cron blocker. **Design-pass note:** weigh the blast radius before granting the cron `actions: write` / a `secrets:write` PAT — that credential can rewrite *any* repo secret. Consider a dedicated fine-grained PAT scoped to only this repo's secrets, or an external token store (e.g. encrypted blob in the vault repo the deploy key already writes), as lower-blast-radius alternatives.

**Update 2026-06-02 (IMPLEMENTED on branch `fix/cron-token-rotation`, pending merge):** Design pass + /plan-eng-review + Codex outside-voice done. **Correction to the original plan:** the built-in `GITHUB_TOKEN` CANNOT write Actions secrets at any permission level — `actions: write` covers runs/artifacts/caches, NOT secrets. Writing a repo secret requires a PAT. Chosen model: a **fine-grained PAT** (`Secrets:write` on this repo only), stored as the `XSENSAI_SECRETS_PAT` secret, shelled out via `gh secret set` (value via stdin, no argv leak; no `--body -`). New module `src/xsensai/sync/gh_secrets_updater.py` + `GhSecretTokenProvider(EnvSecretTokenProvider)` in `auth.py` (catches write failure → `last_persist_error`, never raises mid-fetch). Headless: provider selection (FATAL in `GITHUB_ACTIONS` if PAT missing, opt-out `XSENSAI_ALLOW_NO_PERSIST=1`); `--check` canary write/delete PROVES the PAT works before any X token is consumed; `last_persist_error` gate on ALL success returns → `SYNC_TOKEN_PERSIST_FAILED.md` + heartbeat fail + exit 1. New error codes `GH_SECRET_WRITE_FAILED` + `TOKEN_PERSIST_FAILED`. `::add-mask::` on the mid-run rotated token. +31 tests (all offline, `gh` mocked). Docs: CRON_SETUP.md#token-rotation (incl. blast-radius + crash-window + considered-rejected encrypted-blob). **Accepted limitations:** (a) crash between X-consume and secret-write is irreducible; (b) local `/xsync` vs cron token divergence — see follow-ups below.

### Cron `headless.run()` treats `status="empty"` as failure (no-new-bookmarks always exits 2) — FIXED in commit f3565b1
...
### Symmetric dual-write: local `/xsync` should also persist to the GH secret

**Priority:** P1
**Origin:** /plan-eng-review + Codex outside-voice on the cron token-rotation fix (2026-06-02). The cron now self-rotates (persists rotated tokens to the GH secret via PAT), but local `/xsync` (KeychainTokenProvider) writes only to the Mac Keychain. Since X refresh tokens are single-use, a manual `/xsync` between cron runs rotates the token in Keychain and leaves the GH secret stale → next cron dies `AUTH_FAILED`. Interim mitigation shipped: `/xsync` logs a re-push reminder on rotation (`auth.py` KeychainTokenProvider.store + commands/xsync.md note).
**Description:** Make `KeychainTokenProvider.store_refresh_token` (or a wrapper) ALSO push to the GH secret when a local fine-grained PAT is configured (e.g. read from Keychain account `x-api-secrets-pat`, gated behind an opt-in so non-cron users aren't forced to hold a PAT locally). Reuse `gh_secrets_updater.update_repo_secret`. Closes the divergence so mixed local+cron usage never needs a manual re-push. Blast radius: the `Secrets:write` PAT then also lives in the Mac Keychain.
**Files:** [src/xsensai/sync/auth.py](src/xsensai/sync/auth.py), [src/xsensai/sync/gh_secrets_updater.py](src/xsensai/sync/gh_secrets_updater.py), [commands/xsync.md](commands/xsync.md).

### Canary integration test for GH secret propagation

**Priority:** P3
**Origin:** Codex outside-voice #3 (2026-06-02). There's no official GitHub guarantee that a secret updated by run N is visible to an already-queued run N+1. The 2-day cron cadence + the `xsensai-sync` concurrency group make this near-zero in practice, but it's an untested assumption.
**Description:** A gated integration test (NOT in the default lane — it does real GH API writes): set `XSENSAI_X_REFRESH_TOKEN` to a random value, immediately `gh workflow run` with the expected `sha256` as a dispatch input, and have a tiny workflow step compare `sha256($SECRET)` to the input WITHOUT printing the secret. Confirms propagation timing empirically.
**Files:** new gated test + a minimal `.github/workflows/` canary, or a `workflow_dispatch` input on `sync.yml`.
