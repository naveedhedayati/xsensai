...
11. **v0.9.1.1 hot-fix (2026-04-29)** — cron `headless.run()` now treats `status="empty"` as success per spec line 82. Pre-fix, every no-new-bookmarks cron run incorrectly exited 2 + wrote `[INFO/SYNC_DONE]` into `last_error` + climbed `consecutive_cron_failures` toward the banner threshold. Surfaced during manual QA pass; verified live against cron run [25131162287](https://github.com/naveedhedayati/xsensai/actions/runs/25131162287). Not a slice — pure bug fix on top of v0.9.1.0. **Shipped (v0.9.1.1).**
